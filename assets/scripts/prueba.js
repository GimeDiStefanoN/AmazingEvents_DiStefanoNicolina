...
async function upTable() {
    ...
    const eventAgrupados = upCategorias.map(categoria => upcomingEvents.filter( e => e.category == categoria)) //por cada categoria, agrupo los eventos correspondientes en un array diferente
    
    resumenUp = eventAgrupados.map( categoria => { //recorro cada array y traigo el nombre y le calculo la ganancia y % asistencia.
...
divPastEvents.innerHTML = `
    <tr>
        ...
        <td> ganancia de la categoria </td>
        <td> % de asistencia </td>
    </tr>
    `;
