...
function superFiltro() {
    let filtroBusqueda =searchName(pastEvents, inputSearch.value);
...
let pastEvents = data.events.filter(dat => dat.date < currentDate) //comparo fechas y creo array nuevo de eventos con fechas pasadas
...
renderCards(pastEvents); //llamo a la funcion > muestro las cards
...
function filterCategory(selector) {
    ...
    const eventosFiltrados = pastEvents.filter(evento => categorias.includes(evento.category)); //filtro los event por las categorias elegidas

    if (categorias.length > 0) {
        return eventosFiltrados; //si coincide muestro los filtrados
    }
    return pastEvents; //sino, todos los pasados
}